...
- **Choice**: Use `__wasm__` preprocessor macro to decide insertion of `import_module` & `import_name` attributes into the APEX header.  
  **Reason**: This allows the header file to be used for non-Wasm compilation too, opening the door for future experiments. At the same time, compatibility with other emerging compilers is increased, as they may use different syntax for Wasm import annotations.
